...
class BaseCrud:
    ...
    async def get_by_id(cls, session: AsyncSession, id: UUID | int ) -> Schema | None:
        result = await session.execute(select(cls.base_model).where(cls.base_model.id == id))
        obj = result.scalar_one_or_none()
        return cls.get_schema.model_validate(obj)

    @classmethod 
    async def get_filtered_by_param(cls, session: AsyncSession, **kwargs) -> Schema | None:
    ...
    @classmethod
    async def update(cls, session: AsyncSession, record_id: UUID | int, **kwargs)-> Schema:

...
books = APIRouter(prefix="/api/books")


@books.get("/get_all")
async def get_all_books(session: AsyncSession = Depends(get_session)):
    try:
        result = await BookCrud.get_all(session=session)
        if not result:
            raise HTTPException(
...
async def get_book_by_name(name: str, session: AsyncSession = Depends(get_session)):
    try:
        result = await BookCrud.get_obj_by_param(session=session,name=name)
        if not result:
            raise HTTPException(
...
@books.post("/create")
async def create_book(book: CreateBookSchema, session: AsyncSession = Depends(get_session)):
    try:
        ...
        new_book = await BookCrud.create(session, name=book.name,description=book.description, avaliable_copies=book.avaliable_copies,author_id=book.author)
        await BookGenreCrud.create(session=session, genres=get_genre.id, books=new_book.id)
...
@books.put("/update")
async def update_book(id: UUID, data: UpdateBookSchema, session: AsyncSession = Depends(get_session)):
    try:
        result = await BookCrud.update(session=session,record_id=id, **data.model_dump())
        if not result:
            raise HTTPException(
...
async def delete_book(id: UUID, session: AsyncSession = Depends(get_session)):
    try:
        get_book_genre_assotiation = await BookGenreCrud.get_filtered_by_param(session=session,books=id)
